...
def x1_sampler(samples: int) -> Tensor:
    x1 = torch.rand(samples) * 4 - 2
    x2_ = torch.rand(samples) - torch.randint(high=2, size=(samples,)) * 2
    x2 = x2_ + (torch.floor(x1) % 2)

    data = 1.0 * torch.cat([x1[:, None], x2[:, None]], dim=1) / 0.45

    data = (data + 5.0) / 10 * 2.0 - 1.0
...
def main():
    ...
    for i, sol in enumerate(sols):
        H = axs[i].hist2d(sol[:, 0], sol[:, 1], 300, range=((-1, 1), (-1, 1)))
        ...
        _ = axs[i].hist2d(
            sol[:, 0], sol[:, 1], 300, range=((-1, 1), (-1, 1)), norm=norm
        )
    ...
    grid_size = 200
    grid_axis = torch.linspace(-1.5, 1.5, grid_size)
    ...
    norm = cm.colors.Normalize(vmax=1 / 32, vmin=0.0)
    plt.imshow(
        log_p1, extent=(-1.5, 1.5, -1.5, 1.5), cmap="viridis", norm=norm, origin="lower"
    )
    plt.show()
